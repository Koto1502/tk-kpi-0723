...
- [`index.html`](index.html) — **the hub**: a hand-written landing page that links the
  reports below and carries the standing caveats. It is *not* generated; edit it directly.
- [`kpi.html`](kpi.html) — KPI overview (DAU, revenue, retention, ad funnel, levels, geo)
...
- [`stages.html`](stages.html) — campaign stage clear times, progression speed, and
  purchases against stage reached (generated by `dashboard/refresh_stages.py`)

Every report carries an "← All reports" link back to the hub.
...
`kpi.html` is generated from [`template.html`](template.html) (the page with a
...
Then commit the updated `kpi.html`. (`index.html` is the hub and is never regenerated
by `generate.py` — if `template.html` is ever missing, the generator bootstraps it from
`kpi.html`, not from the hub.)
