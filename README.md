...
`retention.html` is generated by `dashboard/build_retention_cohorts.py` in the parent
Mirror repo (not in this repo, because it reads the gitignored raw AppsFlyer install
CSVs) and copied here. It joins AppsFlyer installs to GA4 activity by
`user_property.appsflyer_id`; only aggregate counts are embedded in the page.
